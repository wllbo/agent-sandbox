...
func newTestClient(opts Options) (*SandboxClient, *fakeagents.Clientset, *fakeextensions.Clientset) {
	...
	return c, agentsCS, extensionsCS
}

func readySandbox(name, namespace string) *sandboxv1alpha1.Sandbox {
...
// The sandbox name is set to match the claim name, mirroring the
// controller's behavior (the field selector metadata.name=<claimName>
// relies on this invariant, but K8s fakes do not enforce field selectors).
func setupWatchWithReactor(agentsCS *fakeagents.Clientset, extensionsCS *fakeextensions.Clientset, sb *sandboxv1alpha1.Sandbox) *watch.FakeWatcher {
	fakeWatcher := watch.NewFake()
	agentsCS.PrependWatchReactor("sandboxes", ktesting.DefaultWatchReactor(fakeWatcher, nil))

	// When the claim is created, immediately send the sandbox ready event
	// with the sandbox name matching the claim name.
...
func TestNewClient_MissingTemplateName(t *testing.T) {
...
func TestOpen_CreatesClaimAndBecomesReady(t *testing.T) {
	...
	c, agentsCS, extensionsCS := newTestClient(opts)
	setupWatchWithReactor(agentsCS, extensionsCS, readySandbox("test-sandbox", "default"))
	...
	if c.baseURL != opts.APIURL {
		t.Errorf("expected baseURL=%s, got %s", opts.APIURL, c.baseURL)
	}
}

func TestOpen_ListBeforeWatch_AlreadyReady(t *testing.T) {
	opts := defaultTestOpts()
	c, agentsCS, _ := newTestClient(opts)

	// Pre-populate a ready sandbox so the List path finds it before Watch.
	sb := readySandbox("test-sandbox", "default")
	...
	// Watch should not be needed, but provide one in case List returns empty.
	fakeWatcher := watch.NewFake()
	agentsCS.PrependWatchReactor("sandboxes", ktesting.DefaultWatchReactor(fakeWatcher, nil))
	...
	if c.SandboxName() != "test-sandbox" {
		t.Errorf("expected SandboxName from List path, got %s", c.SandboxName())
	}
}
...
func TestClose_DeletesClaim(t *testing.T) {
	opts := defaultTestOpts()
	c, agentsCS, extensionsCS := newTestClient(opts)
	setupWatchWithReactor(agentsCS, extensionsCS, readySandbox("sb", "default"))
...
func TestModeSelection_DirectURL(t *testing.T) {
	...
	if c.baseURL != "http://direct.example.com" {
		t.Errorf("expected direct URL mode, got baseURL=%s", c.baseURL)
	}
}
...
func TestModeSelection_APIURLTakesPrecedence(t *testing.T) {
	...
	if c.baseURL != "http://direct.example.com" {
		t.Errorf("expected APIURL to take precedence, got baseURL=%s", c.baseURL)
	}
}
...
func TestIsReady_DoesNotBlockDuringOpen(t *testing.T) {
	...
	go func() {
		_ = c.Open(context.Background()) // will timeout after 500ms
		close(done)
	}()

	// IsReady should return immediately (not blocked by Open holding c.mu).
	ready := make(chan bool, 1)
	go func() {
		time.Sleep(50 * time.Millisecond) // let Open start
		ready <- c.IsReady()
	}()
...
func TestResolveRouterPod_Success(t *testing.T) {
	...
	pod, err := c.resolveRouterPod(context.Background())
	if err != nil {
...
func TestResolveRouterPod_NoEndpoints(t *testing.T) {
	...
	_, err := c.resolveRouterPod(context.Background())
	if err == nil {
...
func TestModeSelection_Gateway(t *testing.T) {
	...
	c := newClientFromInterfaces(opts, agentsCS.AgentsV1alpha1(), extensionsCS.ExtensionsV1alpha1(), dynCS, nil, nil, nil)

	// Wire sandbox readiness.
	sandboxWatcher := watch.NewFake()
	agentsCS.PrependWatchReactor("sandboxes", ktesting.DefaultWatchReactor(sandboxWatcher, nil))
	extensionsCS.PrependReactor("create", "sandboxclaims", func(action ktesting.Action) (bool, runtime.Object, error) {
		go sandboxWatcher.Add(readySandbox("sb", "default"))
		return false, nil, nil
	})

	// Wire gateway IP discovery: send the event as soon as the watch is established.
	...
	if c.baseURL != "http://203.0.113.10" {
		t.Errorf("expected gateway IP in baseURL, got %s", c.baseURL)
	}
}
...
func TestExtractGatewayAddress(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := extractGatewayAddress(gw); got != tc.expected {
...
func TestGatewayDiscovery_ListUsesCorrectFieldSelector(t *testing.T) {
	...
	// Return the gateway from list so the watch path is not needed.
	var capturedSelector string
...
// Verify gatewayGVR matches the constants used in gateway.go.
...
func TestOpen_RollbackDeletesClaim(t *testing.T) {
	...
	// Verify the claim was cleaned up during rollback.
	if c.ClaimName() != "" {
...
func TestClose_DeleteClaim_NotFound(t *testing.T) {
	...
	// Pre-delete the claim so the API returns NotFound.
	claimName := c.ClaimName()
...
func TestSetGatewayURL_IPv6(t *testing.T) {
	opts := defaultTestOpts()
	c, _, _ := newTestClient(opts)

	c.setGatewayURL("2001:db8::1")
	if c.baseURL != "http://[2001:db8::1]" {
		t.Errorf("expected http://[2001:db8::1], got %s", c.baseURL)
	}

	c.setGatewayURL("10.0.0.1")
	if c.baseURL != "http://10.0.0.1" {
		t.Errorf("expected http://10.0.0.1, got %s", c.baseURL)
	}
}
...
func TestOpen_CloseReopens(t *testing.T) {
	...
	// Use a shared variable so the create reactor always sends to the
	// currently-active watcher, avoiding sends on a closed channel.
	var mu sync.Mutex
	...
	extensionsCS.PrependReactor("create", "sandboxclaims", func(action ktesting.Action) (bool, runtime.Object, error) {
		...
		w := currentWatcher
		mu.Unlock()
		go w.Add(readySandbox("sb", "default"))
		return false, nil, nil
	})
	...
	// Swap in a fresh watcher for the second Open.
	mu.Lock()
	currentWatcher = watch.NewFake()
	mu.Unlock()
...
func TestOpen_CreateClaimFailure(t *testing.T) {
	opts := defaultTestOpts()
	c, _, extensionsCS := newTestClient(opts)
...
// --- watch.Error / 410 Gone handling ---

func TestDrainSandboxWatch_410Gone(t *testing.T) {
	opts := defaultTestOpts()
	c, _, _ := newTestClient(opts)

	fw := watch.NewFake()
	go func() {
		fw.Error(&metav1.Status{Code: 410, Reason: metav1.StatusReasonGone})
	}()

	var lastCond string
	done, err := c.drainSandboxWatch(context.Background(), fw, &lastCond)
	if done {
		t.Error("expected done=false for 410 Gone")
	}
	if err != nil {
		t.Errorf("expected nil error for 410 Gone (should trigger re-list), got: %v", err)
	}
}

func TestDrainSandboxWatch_NonGoneError(t *testing.T) {
	opts := defaultTestOpts()
	c, _, _ := newTestClient(opts)

	fw := watch.NewFake()
	go func() {
		fw.Error(&metav1.Status{Code: 500, Message: "internal error"})
	}()

	var lastCond string
	done, err := c.drainSandboxWatch(context.Background(), fw, &lastCond)
	if done {
		t.Error("expected done=false for watch error")
	}
	// Non-410 watch errors are recoverable (return nil) so the outer
	// loop re-lists with backoff rather than aborting.
	if err != nil {
		t.Errorf("expected nil error for transient watch error (should trigger re-list), got: %v", err)
	}
}
...
func TestDrainSandboxWatch_Deleted(t *testing.T) {
	...
	fw := watch.NewFake()
	go func() {
		fw.Delete(readySandbox("sb", "default"))
	}()
...
func TestOpen_ContextCancelledAtSemaphore(t *testing.T) {
	...
	// Manually hold the lifecycle semaphore.
...
func TestClose_ContextCancelledAtSemaphore(t *testing.T) {
	opts := defaultTestOpts()
	opts.CleanupTimeout = 100 * time.Millisecond // short timeout for the detached semaphore wait
	c, _, _ := newTestClient(opts)
...
func TestClose_CallerCtxCancelledWhileSemaphoreHeld(t *testing.T) {
	opts := defaultTestOpts()
	opts.CleanupTimeout = 5 * time.Second // long ceiling
	c, _, _ := newTestClient(opts)

	// Hold the semaphore to simulate a concurrent Open.
	c.lifecycleSem <- struct{}{}
...
func TestClose_CancelsBlockedOpen(t *testing.T) {
	...
	// Watcher that never sends a ready sandbox.
	fakeWatcher := watch.NewFake()
	agentsCS.PrependWatchReactor("sandboxes", ktesting.DefaultWatchReactor(fakeWatcher, nil))

	// Signal when claim is created so we know Open is blocked in waitForSandboxReady.
	...
	// Wait for the claim to be created before calling Close.
	<-claimCreated

	// Close cancels Open's blocked wait and cleans up.
	if err := c.Close(context.Background()); err != nil {
		t.Fatalf("Close() error: %v", err)
	}
...
func TestMonitorPortForward_ClearsStateOnDeath(t *testing.T) {
	...
	go c.monitorPortForward(errChan, stderrBuf, "test-pod", stopChan)

	errChan <- fmt.Errorf("connection lost")

	// Poll for the monitor to process.
	...
	c.mu.Lock()
	defer c.mu.Unlock()
	if !errors.Is(c.lastError, ErrPortForwardDied) {
		t.Errorf("expected ErrPortForwardDied in lastError, got %v", c.lastError)
	}
}
...
func TestMonitorPortForward_StaleMonitorIgnored(t *testing.T) {
	...
	c.mu.Lock()
	c.baseURL = "http://127.0.0.1:12345"
	c.portForwardStopChan = newStopChan
	c.mu.Unlock()

	done := make(chan struct{})
	go func() {
		c.monitorPortForward(errChan, stderrBuf, "test-pod", oldStopChan)
		close(done)
	}()

	errChan <- fmt.Errorf("old connection lost")
	<-done // wait for monitor goroutine to complete
...
func TestOpen_WatchUsesCorrectFieldSelector(t *testing.T) {
	...
	agentsCS.PrependWatchReactor("sandboxes", func(action ktesting.Action) (bool, watch.Interface, error) {
		...
		capturedSelector = wa.GetWatchRestrictions().Fields.String()
		fw := watch.NewFake()
		go fw.Add(readySandbox("test-sandbox", "default"))
		return true, fw, nil
	})
...
func TestOpen_CreateClaimSendsCorrectTemplate(t *testing.T) {
	...
	// Wire claim creation to capture template and trigger sandbox readiness.
	extensionsCS.PrependReactor("create", "sandboxclaims", func(action ktesting.Action) (bool, runtime.Object, error) {
		...
		go fakeWatcher.Add(readySandbox("sb", "default"))
		return false, nil, nil
	})
...
func TestOpen_RollbackFailure_PreservesClaimName(t *testing.T) {
	...
	// Make delete fail during rollback.
	extensionsCS.PrependReactor("delete", "sandboxclaims", func(action ktesting.Action) (bool, runtime.Object, error) {
	...
	// ClaimName should be preserved so Close() can retry.
	if c.ClaimName() == "" {
...
func TestClose_DeleteFailure_RetainsClaimName(t *testing.T) {
	...
	// ClaimName should be retained so a retry Close() can attempt deletion.
	if c.ClaimName() != claimName {
		t.Errorf("expected ClaimName %q to be retained after failed Close, got %q", claimName, c.ClaimName())
	}
	// SandboxName should be cleared on failed Close to prevent stale identity
	// state. The user should retry Close() to clean up the orphaned claim.
	if c.SandboxName() != "" {
		t.Error("expected SandboxName to be cleared after failed Close")
	}
}

// --- Gateway watch drain error paths ---

func TestDrainGatewayWatch_410Gone(t *testing.T) {
	opts := defaultTestOpts()
	c, _, _ := newTestClient(opts)
	...
	done, err := c.drainGatewayWatch(context.Background(), fw)
	if done {
		t.Error("expected done=false for 410 Gone")
	}
	if err != nil {
...
func TestDrainGatewayWatch_NonGoneError(t *testing.T) {
	opts := defaultTestOpts()
	c, _, _ := newTestClient(opts)
...
func TestWaitForGatewayIP_NilDynamicClient(t *testing.T) {
	...
	err := c.waitForGatewayIP(context.Background())
	if err == nil {
...
func TestStartPortForward_NilCoreClient(t *testing.T) {
	...
	c := newClientFromInterfaces(opts, nil, nil, nil, nil, nil, nil)
	err := c.startPortForward(context.Background())
	if err == nil {
...
func TestGatewayDiscovery_Timeout_CleansUpClaim(t *testing.T) {
	...
	// Verify claim was cleaned up during rollback.
	found := false
	for _, action := range extensionsCS.Actions() {
		if action.GetVerb() == "delete" && action.GetResource().Resource == "sandboxclaims" {
			found = true
...
func TestOpen_ReconnectsAfterTransportDeath(t *testing.T) {
	opts := defaultTestOpts()
	c, agentsCS, extensionsCS := newTestClient(opts)
	sb := readySandbox("sb", "default")
	setupWatchWithReactor(agentsCS, extensionsCS, sb)

	// Allow Get to return the sandbox for verifySandboxAlive.
	...
	// Open() should reconnect (re-discover API URL) instead of returning ErrAlreadyOpen.
	if err := c.Open(context.Background()); err != nil {
	...
	if c.baseURL != opts.APIURL {
		t.Errorf("expected baseURL=%s after reconnect, got %s", opts.APIURL, c.baseURL)
	}
...
func TestOpen_WatchError_NonContext(t *testing.T) {
	...
	// Transient watch errors are retried; persistent failures result in timeout.
	if !errors.Is(err, ErrTimeout) {
...
func TestSyncBuffer_Truncation(t *testing.T) {
	...
	// Write more — should report full length but not store.
...
func TestSyncBuffer_PartialTruncation(t *testing.T) {
	...
	// Write more than remaining space — reports original len, stores partial.
...
func TestOpen_ReconnectsAfterTransportDeath_GatewayMode(t *testing.T) {
	...
	extensionsCS.PrependReactor("create", "sandboxclaims", func(action ktesting.Action) (bool, runtime.Object, error) {
		go sandboxWatcher.Add(sb)
		return false, nil, nil
	})

	// Wire gateway discovery — returns a different IP each time.
	var gwMu sync.Mutex
	gwCallCount := 0
	dynCS.PrependWatchReactor("gateways", func(action ktesting.Action) (bool, watch.Interface, error) {
		fw := watch.NewFake()
	...
	if c.baseURL != "http://203.0.113.1" {
		t.Errorf("expected first gateway IP, got %s", c.baseURL)
	}
	...
	// Reconnect should discover new gateway IP.
	if err := c.Open(context.Background()); err != nil {
		t.Fatalf("Open() reconnect error: %v", err)
	}
	if c.baseURL != "http://203.0.113.2" {
		t.Errorf("expected second gateway IP after reconnect, got %s", c.baseURL)
	}
	c.Close(context.Background())
}
...
func TestGatewayDiscovery_ListPath_AlreadyReady(t *testing.T) {
	...
	// Pre-populate gateway in list so watch is not needed.
	dynCS.PrependReactor("list", "gateways", func(action ktesting.Action) (bool, runtime.Object, error) {
		return true, &unstructured.UnstructuredList{
			Object: map[string]interface{}{"apiVersion": "gateway.networking.k8s.io/v1", "kind": "GatewayList"},
			Items: []unstructured.Unstructured{{
	...
	if c.baseURL != "http://198.51.100.1" {
		t.Errorf("expected gateway IP from list path, got %s", c.baseURL)
	}
}
...
func TestHTTPError_CanBeExtracted(t *testing.T) {
	...
	// Use a real HTTP server that returns 404.
...
func TestOpen_OrphanedClaimReturnsError(t *testing.T) {
	...
	// Simulate orphaned state: claimName set, sandboxName empty.
	c.mu.Lock()
...
func TestOpen_ReconnectFailsWhenSandboxGone(t *testing.T) {
	...
	// Get reactor returns NotFound to simulate sandbox disappearing.
	agentsCS.PrependReactor("get", "sandboxes", func(action ktesting.Action) (bool, runtime.Object, error) {
		return true, nil, k8serrors.NewNotFound(
	...
	// Simulate transport death: clear baseURL but keep claim/sandbox.
	c.mu.Lock()
	c.baseURL = ""
	c.mu.Unlock()

	// Reconnect should fail because sandbox is gone and transition to orphaned state.
	err := c.Open(context.Background())
	if err == nil {
	...
	// Verify state: claimName preserved for Close() retry, sandboxName cleared.
	if c.ClaimName() != claimName {
...
func TestOpen_ReconnectFailsWhenSandboxNotReady(t *testing.T) {
	...
	// Simulate port-forward death: clear baseURL but keep claim/sandbox.
	c.mu.Lock()
	c.baseURL = ""
	c.mu.Unlock()

	// Override sandbox Get to return not-ready.
	agentsCS.PrependReactor("get", "sandboxes", func(action ktesting.Action) (bool, runtime.Object, error) {
	...
	// claimName preserved; sandboxName preserved because the error is
	// not-ready (not NotFound). Only NotFound clears sandboxName.
	if c.ClaimName() != claimName {
		t.Errorf("expected claimName %q preserved, got %q", claimName, c.ClaimName())
	}
	if c.SandboxName() == "" {
...
func TestOpen_ReconnectFailsWhenClaimGone(t *testing.T) {
	...
	// Set up as if previously opened and port-forward died.
	c.mu.Lock()
	c.claimName = "test-claim-abc"
	c.sandboxName = "test-sandbox"
	c.mu.Unlock()

	// Mock claim Get to return NotFound.
	...
	// When a 404 is returned, the client should reset to clean state
	// (not orphaned) so the next Open() can create a fresh sandbox.
	if errors.Is(err, ErrOrphanedClaim) {
		t.Errorf("expected non-orphan error for 404 claim, got: %v", err)
	}
	if !strings.Contains(err.Error(), "claim was deleted") {
		t.Errorf("expected 'claim was deleted' error, got: %v", err)
	}
	// Verify state: fully reset so next Open() starts fresh.
	if c.ClaimName() != "" {
...
func TestClose_CancelledContextUsesDetachedCleanup(t *testing.T) {
	...
	// Cancel the context before calling Close.
	ctx, cancel := context.WithCancel(context.Background())
	cancel()

	// Close should still succeed via detached context.
...
// --- setSandboxState clears stale annotations ---

func TestSetSandboxState_ClearsStaleAnnotations(t *testing.T) {
	opts := defaultTestOpts()
	c, _, _ := newTestClient(opts)

	// First call sets annotations.
	sbWithAnnotations := &sandboxv1alpha1.Sandbox{
		ObjectMeta: metav1.ObjectMeta{
			Name: "sb1",
			Annotations: map[string]string{
	...
	c.setSandboxState(sbWithAnnotations)
	if c.Annotations() == nil {
		t.Fatal("expected non-nil annotations after first setSandboxState")
	}
	if c.Annotations()["custom"] != "value" {
		t.Errorf("expected custom annotation, got %v", c.Annotations())
	}

	// Second call with nil annotations should clear them.
	sbNoAnnotations := &sandboxv1alpha1.Sandbox{
		ObjectMeta: metav1.ObjectMeta{Name: "sb2"},
	}
	c.setSandboxState(sbNoAnnotations)
	if c.Annotations() != nil {
		t.Errorf("expected nil annotations after setSandboxState with nil, got %v", c.Annotations())
	}
	if c.PodName() != "sb2" {
...
func TestOpen_WatchClosedReListFinds(t *testing.T) {
	...
	// First watch: close immediately to trigger re-list.
	// Second watch: send the ready sandbox (fallback in case list misses).
	...
	// List returns empty on first call, then returns the ready sandbox.
	...
	if c.SandboxName() != "test-sandbox" {
		t.Errorf("expected SandboxName=test-sandbox, got %s", c.SandboxName())
	}
}
...
func TestMonitorPortForward_Timeout(t *testing.T) {
	...
	go func() {
		c.monitorPortForward(errChan, stderrBuf, "test-pod", stopChan)
		close(done)
	}()

	// Close stop channel but never send to errChan.
	close(stopChan)

	// Monitor should exit after monitorExitTimeout.
	select {
	case <-done:
		// Success — monitor exited after timeout.
	case <-time.After(monitorExitTimeout + 5*time.Second):
		t.Fatal("monitorPortForward did not exit after stop signal timeout")
	}

	// baseURL should be cleared by the monitor timeout path so that
	// subsequent operations fail fast with ErrNotReady instead of
	// hanging on a dead port-forward endpoint.
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.baseURL != "" {
		t.Errorf("expected baseURL cleared after monitor timeout, got %q", c.baseURL)
	}
	// lastError should be set defensively on the timeout path.
	if c.lastError == nil {
...
func TestDrainGatewayWatch_NonUnstructuredObject(t *testing.T) {
	...
	fw := watch.NewFake()
	go func() {
		// Send an object that is not *unstructured.Unstructured.
...
func TestMonitorPortForward_Timeout_SetsLastError(t *testing.T) {
	...
	// Close stop channel but never send to errChan — triggers timeout path.
	close(stopChan)
	...
	if c.lastError != nil && !errors.Is(c.lastError, ErrPortForwardDied) {
		t.Errorf("expected ErrPortForwardDied in lastError, got: %v", c.lastError)
	}
}
...
func TestClose_DeleteFailure_ClearsIdentityButKeepsClaim(t *testing.T) {
	...
	// claimName preserved so Close() can be retried.
	if c.ClaimName() != claimName {
		t.Errorf("expected ClaimName %q to be preserved, got %q", claimName, c.ClaimName())
	}
	// Identity state cleared to prevent stale accessors.
	if c.SandboxName() != "" {
		t.Error("expected SandboxName to be cleared after failed Close")
	}
	if c.PodName() != "" {
	...
	// Open should return ErrOrphanedClaim, directing user to retry Close().
...
func TestClose_DrainsInflightBeforeDelete(t *testing.T) {
	...
	extensionsCS.PrependReactor("delete", "sandboxclaims", func(action ktesting.Action) (bool, runtime.Object, error) {
		select {
		case <-opDone:
			// Good: operation completed before delete.
		default:
	...
	go func() {
		c.Exists(context.Background(), "test.txt")
		close(opDone)
	}()

	<-requestReceived
	// Unblock after a delay so Close actually has to drain.
...
func TestOpen_ReconnectTransportFailure(t *testing.T) {
	...
	// Pre-set state: previously opened client whose transport died.
	...
	agentsCS.PrependReactor("get", "sandboxes", func(action ktesting.Action) (bool, runtime.Object, error) {
		return true, readySandbox("existing-sandbox", "default"), nil
	})
	// Gateway returns no address → timeout during reconnect.
	dynCS.PrependReactor("list", "gateways", func(action ktesting.Action) (bool, runtime.Object, error) {
		return true, &unstructured.UnstructuredList{
			Object: map[string]interface{}{"apiVersion": "gateway.networking.k8s.io/v1", "kind": "GatewayList"},
		}, nil
...
func TestOpen_ReconnectTransientClaimError(t *testing.T) {
	...
	// Pre-set state: previously opened client.
	c.claimName = "existing-claim"
	c.sandboxName = "existing-sandbox"
